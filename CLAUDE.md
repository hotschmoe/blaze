...
cd test && zig build   # Build conformance test suite
```
...
- **Comptime Shader Reflection** - WGSL parsing at comptime, type-safe bind group layouts
- **Comptime Pipeline Configuration** - Vertex layouts from struct fields, no Vulkan boilerplate
- **Tagged Unions for Commands** - Type-safe command recording
- **Timeline Semaphores** - Modern synchronization as primary sync primitive
...
Foundation layer of the zig-graphics stack. FORGE and FLUX build on top of BLAZE.
